...
install_packages() {
    ...
    echo -e "${GREEN}✓ Latest kernel installed"
    
    if [[ "$STEAMOS_VERSION" == "3.7" ]]; then
        echo -e "${YELLOW}Installing 3.7-specific packages...${NC}"
        pacstrap -K /mnt --needed --noconfirm steamos-powerbuttond || true
    fi
    
    echo -e "${GREEN}✓ 3.7 Specific packages installed"
}
...
# Create user
useradd -m -s /usr/bin/zsh -G polkitd,geoclue,flatpak,rfkill,video,render,input,audio,wheel,power,network,games __USERNAME__
